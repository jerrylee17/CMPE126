...
public:
	...
	unsigned int seqSearchRec(elemType element, int st = 0);
};
...
template<class elemType>
unsigned int arrayList<elemType>::seqSearchRec(elemType element, int st){
	if (st == size) {
		return -1;
	}
	else if (arr[st] == element) {
		return st;
	}
	return seqSearchRec(element, st + 1);
}
